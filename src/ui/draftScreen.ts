...
import { HERO_DESCRIPTIONS, DRAFT } from '../game/config.ts';
...
export function renderDraftScreen(host: HTMLElement, state: GameState, opts: DraftScreenOpts): void {
  ...
  const detail = (u: Unit): string => {
    ...
    const bars = AGG_LABEL.map(([k, label]) =>
      `<div class="ds-stat"><span class="ds-stat-l">${label}</span>
    ...
    return `<div class="ds-detail-in">
      <div class="ds-chips"><span class="ds-chip">${esc(u.role)}</span><span class="ds-chip">${esc(u.hero)}</span>
        <span class="ds-chip">${esc(u.weapon)}</span>${u.personality ? `<span class="ds-chip">${esc(u.personality)}</span>` : ''}</div>
  ...
  host.innerHTML = `<div class="ds-root">
  ...
  host.querySelectorAll<HTMLButtonElement>('[data-row]').forEach((b) => b.addEventListener('click', () => {
    selectedId = b.getAttribute('data-row');
    renderDraftScreen(host, state, opts);
  }));
  host.querySelectorAll<HTMLButtonElement>('[data-remove]').forEach((b) => b.addEventListener('click', (e) => {
    e.stopPropagation();
    opts.onUnpick(b.getAttribute('data-remove') ?? '');
  }));
  const draftBtn = host.querySelector<HTMLButtonElement>('[data-draft]');
  draftBtn?.addEventListener('click', () => {
    const id = draftBtn.getAttribute('data-draft') ?? '';
    if (pickedIds.has(id)) opts.onUnpick(id);
    else if (remaining > 0) opts.onPick(id);
  });
  host.querySelector<HTMLButtonElement>('[data-confirm]')?.addEventListener('click', () => opts.onConfirm());
}
